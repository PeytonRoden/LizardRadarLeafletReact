...
int MAX_NUM_VOXELS_PER_SIDE = 256;
int num_voxels_latitude = 256;
int num_voxels_longitude = 256;
int num_voxels_height = 256;
...
// Lv_0/Lh_0 act as the near-range floor (roughly the super-res gate spacing, ~0.25-0.5 km).
// k_Lv/k_Lh model beam-width spreading: a 1° beam is ~0.0175 km wide per km of range,
// so sigma grows by ~half of that, ~0.009 per km.
float Lv_0 = 0.70f;
float Lh_0 = 0.70f;
...
// exponent applied to the Gaussian splat weights (w^p). 1 = plain weighted mean,
// 2-4 concentrates weight on near-center bins so compact high-reflectivity
// features (tornado debris balls) are not washed out by the average.
float weight_sharpness = 100.0f;
...
// Nyquist velocity (m/s) from this radial's RADIAL_DATA_BLOCK. Needed for
// dealiasing because it can differ between radials (batch/split cuts).
float read_radial_nyquist_ms(const uint8_t* msg31_ptr, uint32_t block_pointer_3) {
    if (block_pointer_3 == 0) return NAN;
    const uint8_t* p = msg31_ptr + block_pointer_3;
    // block_type(1) + data_name(3) + lrtup(2) + unambig_range(2) + noise_h(4) + noise_v(4)
...
void parse_one_moment(AllTilt& alltilts, const uint8_t* ref_ptr , MSG_31& msg31, const uint8_t* msg31_ptr){
        ...
        const size_t radials_size_before = radials->size();
        ...
        for (int i = 0; i < MOMENT.gate_count; ++i) {
            ...
            // 8-bit code 1 = range folded (second-trip echo); not usable velocity
            if (is_vel && raw_val == 1) continue;
        ...
        // Keep VelNyquist aligned with the rays the dealiaser will derive
        // from Radials_VEL: one entry per VEL radial with >= 1 valid gate.
        if (is_vel && radials->size() > radials_size_before) {
            current_tilt.VelNyquist.push_back(ray_nyq_ms);
            VelocityRay ray{};
            ray.start = radials_size_before / 3;
            ray.count = (radials->size() - radials_size_before) / 3;
            ray.nyquist = ray_nyq_ms;
            ray.gateSpacing = MOMENT.gate_spacing;
            current_tilt.VelocityRays.push_back(ray);
        }
}
...
AllTilt combine_all_tilts_from_thread_results(std::vector<AllTilt>& thread_results) {
    ...
    auto merge_tilt = [&](SingleTilt& dst, const SingleTilt& src) {
        ...
        const size_t velocity_start_offset = dst.Radials_VEL.size() / 3;
        append_radials(dst.Radials_VEL, src.Radials_VEL);
        for (const auto& source_ray : src.VelocityRays) {
            VelocityRay ray = source_ray;
            ray.start += velocity_start_offset;
            dst.VelocityRays.push_back(ray);
        }
    ...
    for (auto& tilt : combined.Tilts) {
        ...
        tilt.VelNyquist.shrink_to_fit();
        tilt.VelocityRays.shrink_to_fit();
    }
...
extern "C" {
    ...
    EMSCRIPTEN_KEEPALIVE
    void set_Lv_0(float v) { Lv_0 = v; }

    EMSCRIPTEN_KEEPALIVE
    void set_Lh_0(float v) { Lh_0 = v; }

    EMSCRIPTEN_KEEPALIVE
    void set_k_Lv(float v) { k_Lv = v; }

    EMSCRIPTEN_KEEPALIVE
    void set_k_Lh(float v) { k_Lh = v; }

    EMSCRIPTEN_KEEPALIVE
    void set_weight_sharpness(float v) { weight_sharpness = v; }

    EMSCRIPTEN_KEEPALIVE
    float get_Lv_0() { return Lv_0; }

    EMSCRIPTEN_KEEPALIVE
    float get_Lh_0() { return Lh_0; }

    EMSCRIPTEN_KEEPALIVE
    float get_k_Lv() { return k_Lv; }

    EMSCRIPTEN_KEEPALIVE
    float get_k_Lh() { return k_Lh; }

    EMSCRIPTEN_KEEPALIVE
    float get_weight_sharpness() { return weight_sharpness; }
