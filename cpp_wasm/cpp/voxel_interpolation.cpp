...
float calculate_Lh(float r) { return std::hypot(Lh_0, k_Lh * r); }
float calculate_Lv(float r) { return std::hypot(Lv_0, k_Lv * r); }
...
std::pair<std::vector<float>, std::vector<int>> get_voxel_weights_and_indices_new(
    ...
    // effective sigma of the sharpened kernel is L / sqrt(weight_sharpness); cut off at 3 effective sigmas
    const float sharp_denom = std::sqrt(std::max(weight_sharpness, 0.000001f));
    const int radius_latitude = std::min({num_voxels_latitude - 1, MAX_VOXEL_RADIUS, static_cast<int>(std::ceil(3.0f * Lh / (sharp_denom * latitude_voxel_size_km)))});
    const int radius_longitude = std::min({num_voxels_longitude - 1, MAX_VOXEL_RADIUS, static_cast<int>(std::ceil(3.0f * Lh / (sharp_denom * longitude_voxel_size_km)))});
    const int radius_height = std::min({num_voxels_height - 1, MAX_VOXEL_RADIUS, static_cast<int>(std::ceil(3.0f * Lv / (sharp_denom * height_voxel_size_km)))});
    ...
    // weight^weight_sharpness == exp(-weight_sharpness * (d/L)^2); sharpness 1 = plain Gaussian,
    // larger values concentrate weight on near-center bins so compact peaks (e.g. a debris ball) survive averaging
    const float sharpened_Lh2 = (Lh * Lh) / weight_sharpness;
    const float sharpened_Lv2 = (Lv * Lv) / weight_sharpness;

    // separable Gaussian: precompute one 1D weight array per axis so the triple loop
    // needs no exp calls at all (3N exp instead of N^3)
    std::vector<float> weights_x(2 * radius_latitude + 1);
    std::vector<float> weights_y(2 * radius_longitude + 1);
    std::vector<float> weights_z(2 * radius_height + 1);
    for (int i = -radius_latitude; i <= radius_latitude; i++) {
        const float d = i * latitude_voxel_size_km;
        weights_x[i + radius_latitude] = std::exp(-(d * d) / sharpened_Lh2);
    }
    for (int j = -radius_longitude; j <= radius_longitude; j++) {
        const float d = j * longitude_voxel_size_km;
        weights_y[j + radius_longitude] = std::exp(-(d * d) / sharpened_Lh2);
    }
    for (int k = -radius_height; k <= radius_height; k++) {
        const float d = k * height_voxel_size_km;
        weights_z[k + radius_height] = std::exp(-(d * d) / sharpened_Lv2);
    }

    const size_t max_neighbors = static_cast<size_t>(2 * radius_latitude + 1) * (2 * radius_longitude + 1) * (2 * radius_height + 1);
    weights.reserve(max_neighbors);
    indices.reserve(max_neighbors);

    for (int i = std::max(0, x_center - radius_latitude); i <= std::min(num_voxels_latitude - 1, x_center + radius_latitude); i++) {
        for (int j = std::max(0, y_center - radius_longitude); j <= std::min(num_voxels_longitude - 1, y_center + radius_longitude); j++) {
            for (int k = std::max(0, z_center - radius_height); k <= std::min(num_voxels_height - 1, z_center + radius_height); k++) {

                int voxel_index = get_voxel_index_new(i, j, k, num_voxels_latitude, num_voxels_longitude, num_voxels_height);
                ...
                weights.push_back(weights_x[i - x_center + radius_latitude] * weights_y[j - y_center + radius_longitude] * weights_z[k - z_center + radius_height]);
