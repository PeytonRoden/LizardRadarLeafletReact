...
// ============================================================================
// v11 = v10 + fixes for (1) disconnected islands flipping and (2) residual
// errors in the tornado couplet.
//
// Pipeline (volume):
//   A. Per-tilt region solve (v2 backbone). New: range gaps up to 3 gates and
//      one missing radial are bridged, so a single dropped gate no longer
//      manufactures an "island". Each disconnected component is tracked.
//      The largest component is anchored with a pair-balanced sector mean
//      (v(az) + v(az+180) ~ 0 for a horizontal wind), not a raw scan mean.
//   V. Volume VAD profile, fitted by robust least squares to ALREADY
//      DEALIASED gates of large components from all low tilts, binned by
//      beam height. Every component (islands included) is then shifted by
//      the single integer fold that best matches that profile, only when
//      the vote is decisive. Two rounds (fit -> anchor -> refit -> anchor).
//      Linear cost. The reference is only used in azimuth sectors (or the
//      opposite sector) that actually had data at that height.
//   B. Region-level vertical reconciliation (unchanged from v10).
//   X. Vortex reference check (after Xu's mesocyclone-targeted dealiasing):
//      clusters of REF-confirmed hard gates -> alias-robust Rankine vortex
//      fit (center, core radius, tangential + radial speed, constant
//      background solved in closed form) -> gates near the core whose fold
//      is clearly wrong against the fitted model are corrected; confident
//      core gates become anchors for C.
...
TiltCache dealias_tilt_stage_a(SingleTilt& tilt) {
    ...
    // NOTE: this is O(regions) per island; islands are usually few. If you
    // see pathological counts, sort region ids by size once instead.
...
VadProfile fit_ar_vad(const AllTilt& volume, const std::vector<TiltCache>& cache) {
    ...
    // Dynamic program over height.
    constexpr double A_START = 0.05, B_SCORE = 20.0, G_TRANS = 0.5;
    std::vector<std::vector<double>> cost(VAD_NBINS);
    std::vector<std::vector<int>> back(VAD_NBINS);
    int prev = -1;
    for (int b = 0; b < VAD_NBINS; ++b) {
        ...
        cost[b].assign(C.size(), 0.0);
        back[b].assign(C.size(), -1);
        for (size_t j = 0; j < C.size(); ++j) {
            const double own = B_SCORE * (best_sc - C[j].score);
            if (prev < 0) {
                cost[b][j] = own + A_START * std::hypot(C[j].U, C[j].V);
            } else {
                const double gap = std::max(1, b - prev);
                double bestc = std::numeric_limits<double>::infinity();
                int arg = -1;
                for (size_t q = 0; q < cands[prev].size(); ++q) {
                    const double tc = cost[prev][q] + G_TRANS *
                        std::hypot(C[j].U - cands[prev][q].U, C[j].V - cands[prev][q].V) / gap;
    ...
    choice.fill(-1);
    {
        int j = static_cast<int>(std::min_element(cost[prev].begin(), cost[prev].end()) - cost[prev].begin());
...
void dealias_velocity_volume_v10_1(AllTilt& volume) {
