...
constexpr float ANCHOR_MAX_MEDIAN_RES = 0.25f;  // in units of Vn
...
TiltCache dealias_tilt_stage_a(SingleTilt& tilt) {
    ...
    //
    // Votes are accumulated incrementally: when a region resolves, its
    // boundary is walked ONCE and each unresolved neighbor's tally is
    // updated. v2/v10 re-scanned a region's whole boundary every time any
    // neighbor resolved (quadratic in region degree); results are the same.
    std::priority_queue<Candidate> pq;
    size_t resolved = 0;
    int current_comp = -1;
    constexpr int NV = 2 * MAX_FOLDS + 1;
    std::vector<std::array<float, NV>> acc(regions.size());
    for (auto& a : acc) a.fill(0.0f);
    std::vector<float> acc_ev(regions.size(), 0.0f);
    std::vector<int> touched_stamp(regions.size(), -1);
    std::vector<int> touched;

    auto push_region = [&](int id) {
        Region& region = regions[id];
        int best = 0;
        for (int i = 1; i < NV; ++i) if (acc[id][i] > acc[id][best]) best = i;
        ++region.version;
        pq.push({acc_ev[id], id, best - MAX_FOLDS, region.version});
    };
    ...
    auto resolve = [&](int id, int fold) {
        ...
        for (const Boundary& edge : region.boundary) {
            ...
            const float edge_nyq = std::min(regions[nb].nyquist, nyq[edge.gate]);
            const int k = fold_from_pair(grid[edge.neighbor], grid[edge.gate], edge_nyq);
            if (std::abs(k) > MAX_FOLDS) continue;
            const float w = sw_conf[edge.gate] * sw_conf[edge.neighbor];
            acc[nb][k + MAX_FOLDS] += w;
            acc_ev[nb] += w;
            if (touched_stamp[nb] != id) { touched_stamp[nb] = id; touched.push_back(nb); }
        }
        for (const int nb : touched) push_region(nb);
    };
    ...
    // Seed components largest-first (one sort, not a scan per island).
    // Component 0 is the main echo.
    std::vector<int> by_size(regions.size());
    std::iota(by_size.begin(), by_size.end(), 0);
    std::sort(by_size.begin(), by_size.end(), [&](int a, int b) {
        if (regions[a].gates.size() != regions[b].gates.size())
            return regions[a].gates.size() > regions[b].gates.size();
        if (regions[a].nyquist != regions[b].nyquist) return regions[a].nyquist > regions[b].nyquist;
        return a < b;
    });
    for (const int seed : by_size) {
...
VadProfile fit_ar_vad(const AllTilt& volume, const std::vector<TiltCache>& cache) {
    constexpr int PER_BIN = 300;
    constexpr float WMAX = 90.0f;
    VadProfile prof;
    ...
    constexpr double JUMP_PER_BIN = 15.0;   // m/s vector change per 500 m
    ...
    for (int b = 0; b < VAD_NBINS; ++b) {
        ...
        for (size_t j = 0; j < C.size(); ++j) {
            ...
            double tc;
            if (last < 0) {
                tc = own + A_START * std::hypot(C[j].U, C[j].V);
            } else {
                const ArCand& pc = cands[last][choice[last]];
                const double jump = std::hypot(C[j].U - pc.U, C[j].V - pc.V);
                if (jump > JUMP_PER_BIN * (b - last)) continue;
                tc = own + G_TRANS * jump / (b - last);
            }
            if (tc < bestc) { bestc = tc; arg = static_cast<int>(j); }
        }
        if (arg < 0) continue;
        choice[b] = arg;
        last = b;
    }
    if (last < 0) return prof;
...
int anchor_components(AllTilt& volume, std::vector<TiltCache>& cache, const VadProfile& vad) {
    ...
    for (size_t ti = 0; ti < volume.Tilts.size(); ++ti) {
        ...
        // Group components that touch through hard (detected-shear) gates.
        // Stage A's relative fold across real shear is kept; the VAD may
        // only move groups separated by missing data (true islands).
        std::vector<int> parent(static_cast<size_t>(c.ncomp));
        std::iota(parent.begin(), parent.end(), 0);
        auto find = [&](int x) { while (parent[x] != x) { parent[x] = parent[parent[x]]; x = parent[x]; } return x; };
        auto unite = [&](int a, int b) { a = find(a); b = find(b); if (a != b) parent[std::max(a, b)] = std::min(a, b); };
        {
            const size_t n = c.hard.size();
            std::vector<unsigned char> seen(n, 0);
            std::vector<int> stack, touched;
            for (size_t s0 = 0; s0 < n; ++s0) {
                if (!c.hard[s0] || seen[s0]) continue;
                stack.assign(1, static_cast<int>(s0));
                seen[s0] = 1;
                touched.clear();
                while (!stack.empty()) {
                    const int g = stack.back(); stack.pop_back();
                    for (int nb : c.graph[g]) {
                        if (nb < 0) continue;
                        if (c.hard[nb]) {
                            if (!seen[nb]) { seen[nb] = 1; stack.push_back(nb); }
                        } else {
                            const int r = c.region_of_gate.empty() ? -1 : c.region_of_gate[nb];
                            if (r >= 0 && c.regions[r].comp >= 0) touched.push_back(c.regions[r].comp);
                        }
                    }
                }
                for (size_t i = 1; i < touched.size(); ++i) unite(touched[0], touched[i]);
            }
        }
        ...
        for (size_t r = 0; r < c.regions.size(); ++r) {
            const int cp0 = c.regions[r].comp;
            if (cp0 < 0) continue;
            const int cp = find(cp0);
        ...
        for (int cp = 0; cp < c.ncomp; ++cp) {
            ...
            std::vector<float> dres;   // (ref - value) / vn per sample
            dres.reserve(ANCHOR_MAX_SAMPLES + 8);
            int nsamp = 0;
            size_t counter = 0;
            for (const int r : comp_regions[cp]) {
                for (const int g : c.regions[r].gates) {
                    ...
                    // Narrow kernel (+/-0.5 Vn): a real fold lands close to
                    // the environmental wind after shifting; a genuine
                    // shear boundary near 2Vn does not.
                    for (int k = -ANCHOR_MAX_FOLD; k <= ANCHOR_MAX_FOLD; ++k) {
                        const double e = std::fabs(d - 2.0 * k * vn) / (0.5 * vn);
                        if (e < 1.0) score[k + ANCHOR_MAX_FOLD] += 1.0 - e;
                    }
                    dres.push_back(static_cast<float>(d / vn));
                    ++nsamp;
                }
            }
            ...
            // Post-shift fit must be tight: median |residual| <= 0.25 Vn.
            // Rejects "uniform-wind looks better if this side were folded"
            // on real shear lines, where the residual stays ~(2Vn-jump)/2.
            for (float& x : dres) x = std::fabs(x - 2.0f * static_cast<float>(k));
            std::nth_element(dres.begin(), dres.begin() + dres.size() / 2, dres.end());
            if (dres[dres.size() / 2] > ANCHOR_MAX_MEDIAN_RES) continue;
...
int vortex_reference_check(AllTilt& volume, std::vector<TiltCache>& cache) {
    ...
    for (size_t ti = 0; ti < volume.Tilts.size(); ++ti) {
        ...
        for (const Center& ctr : centers) {
            ...
            // A window whose folded phases are already nearly uniform has no
            // couplet worth fitting (cheap: one sincos per sample).
            if (score0 >= VORTEX_SKIP_COHERENCE) continue;
            if (fits_done >= VORTEX_MAX_PER_VOLUME) break;
            ++fits_done;
...
void dealias_velocity_volume_v10_2(AllTilt& volume) {
    ...
    // Absolute anchoring: alias-robust VAD from folded phases, then one
    // refinement with an ordinary LS VAD on the now-anchored data.
    {
