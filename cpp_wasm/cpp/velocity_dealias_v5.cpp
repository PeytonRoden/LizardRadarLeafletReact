...
// Region segmentation. 0.50 Nyquist prevents excessive fragmentation while
// still keeping strong shear boundaries from becoming one region.
constexpr float SEG_FRAC = 0.50f;
    ...
    const SingleTilt* sibling)
{
    ...
    const std::vector<float> vertical_aligned =
        prior.valid
            ? align_series(
                  prior.series,
                  simple_rays,
    ...
    // ---------------------------------------------------------------
    // Detect strong shear / TVS-like boundaries.
    //
    // A folded jump is the distance modulo 2*Nyquist. This prevents a
    // normal alias boundary itself from automatically looking like a
    // giant physical velocity shear.
    // ---------------------------------------------------------------
        ...
        [&](int index) -> FoldChoice
    {
        ...
        // A TVS-like region without vertical support needs a strong
        // same-tilt decision before accepting a nonzero fold.
        if (best_k != 0 &&
            region.tvs_like &&
            vertical_support <
                MIN_VERTICAL_SUPPORT_FOR_FOLD &&
...
void dealias_velocity_volume_v5(AllTilt& volume)
{
    // Explicitly sort by elevation rather than depending on ingest order.
