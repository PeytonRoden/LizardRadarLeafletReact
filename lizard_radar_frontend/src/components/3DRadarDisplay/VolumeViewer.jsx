...
export default function VolumeViewer({ voxels, dimensions, renderStyle, isoValue, rayStop, opacity, selectedColorMap, selectedMoment }) {
  ...
  useEffect(() => {
    ...
    // Render on demand instead of a continuous rAF loop. Ray-marching the
    // volume every frame is expensive on mobile GPUs, so we only draw when
    // the camera, data, uniforms, or size change. OrbitControls keeps
    // emitting "change" while damping inertia is active, so motion still
    // animates smoothly.
    let frameId = null;
    let running = true;
    const invalidate = () => {
    ...
    const state = { scene, camera, renderer, controls, volumeResources: null, invalidate };
  ...
  useEffect(() => {
    ...
    if (!voxels || !dimensions || !selectedPalette) {
      state.invalidate();
      return undefined;
    }
  ...
  useEffect(() => {
    const state = stateRef.current;
    const uniforms = state?.volumeResources?.uniforms;
